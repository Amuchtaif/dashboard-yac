...
echo "--- Permits student rooms ---\n";
$stmt = $conn->query("
    SELECT bp.id as permit_id, s.nama_siswa, br.room_name, br.supervisor_id
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    echo "Permit: " . $row['permit_id'] . " Student: " . $row['nama_siswa'] . " Room: " . ($row['room_name'] ?? 'N/A') . " Supervisor: " . ($row['supervisor_id'] ?? 'N/A') . "\n";
}
?>
